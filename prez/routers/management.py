...
async def return_tbox_cache(request: Request):
    ...
    # ``.get("Accept")`` returns None when no Accept header is sent; fall back
    # to an empty string so the default (text/turtle) below is used instead of
    # raising AttributeError. See https://github.com/RDFLib/prez/issues/380.
    mediatype = (request.headers.get("Accept") or "").split(",")[0]
